...
$total_withdrawals_query = "SELECT SUM(amount) as total_withdrawals FROM withdrawals WHERE status = 'processed'";
...
$query = "
    SELECT 'Deposit' AS type,d.gateway,d.id, d.user_id, d.amount, d.status, d.created_at 
    FROM deposits d 
    
    UNION ALL
    
    SELECT 'Withdrawal' AS type,w.gateway,w.id, w.user_id, w.amount, w.status, w.created_at 
    FROM withdrawals w 
    
    UNION ALL
    
    SELECT t.type,t.gateway, t.user_id,t.id, t.amount, t.status, t.created_at 
    FROM transactions t 
